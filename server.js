...
app.post("/order", async (req, res) => {
    try {
        ...
        // ✅ Send real-time update to Deja website
        io.emit("orderStatusUpdated", { orderId: newOrder._id, status: "Order Placed" });
...
// ✅ API to Get Pending Orders for a Restaurant
app.get("/orders/:restaurant", async (req, res) => {
    try {
        ...
        // ✅ Return an empty array instead of 404 if no orders found
        res.json(orders || []);
    } catch (error) {
...
// ✅ API to Update Order Status and Notify Deja Website
app.put("/order-status/:id", async (req, res) => {
    try {
        ...
        // ✅ Send real-time update to Deja website
        io.emit("orderStatusUpdated", { orderId: updatedOrder._id, status: updatedOrder.status });
...
// ✅ Start WebSocket Server Along with Express
